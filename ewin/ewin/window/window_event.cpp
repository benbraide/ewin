...
ewin::window::wnd_event::wnd_event(object &target)
	...
	focus_change(target), enable(target), set_cursor(target), hit_test(target), position_change(target), size(target), move(target),
	style_change(target), unknown_message(target){}
