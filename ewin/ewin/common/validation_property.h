...
namespace ewin::common{
	...
	public:
		...
		explicit validation_property(callback_type callback)
			: callback_(callback){}

		return_type operator [](const value_type &target) const{
			if (callback_ == nullptr)
				throw error_type::uninitialized_property;

			auto info = std::make_pair(&const_cast<value_type &>(target), return_type());
			callback_(const_cast<validation_property *>(this), &info, access_type::read);
	...
	protected:
		...
		friend std::conditional_t<std::is_void_v<manager_type>, validation_property, manager_type>;
