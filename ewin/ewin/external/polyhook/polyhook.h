...
namespace PLH {
	namespace Tools
	{
		...
		inline void* AllocateWithin2GB(uint8_t* pStart, size_t Size, size_t& AllocationDelta)
		{
			...
			if (Allocated != nullptr)
			{
				AllocationDelta = std::abs(pStart - static_cast<uint8_t *>(Allocated));
	...
	protected:
		...
		{
			if (To < From)
				return static_cast<uint8_t>(0 - (From - To) - InsSize);
			return static_cast<uint8_t>(To - (From + InsSize));
		}
...
#define Detour X64Detour
	...
	protected:
		...
		virtual void WriteJMP(uintptr_t From, uintptr_t To) override;
